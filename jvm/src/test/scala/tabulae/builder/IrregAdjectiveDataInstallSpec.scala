...
class IrregAdjectiveDataInstallSpec extends FlatSpec {
  ...
  "The IrregAdjectiveDataInstaller object" should "install adjective data from a single source" in {
    ...
    mkdirs(targetDir)
    assert(targetDir.exists,"IrregAdjectiveDataInstaller:  could not create " + targetDir)

    val targetFile = targetDir / "lexicon-irregverbs.fst"

    IrregAdjectiveDataInstaller(datasets, corpora, targetFile)
    assert(targetFile.exists,"IrregAdjectiveDataInstaller:  did not create " + targetFile)

    val expected = "<u>proof\\.irradj1</u><u>lexent\\.n48627</u>totius<masc><gen><sg><pos><irregadj>"
  ...
  it should "return an empty string if no data are found in the source directory" in  {
    ...
    val output = IrregAdjectiveDataInstaller.fstForIrregAdjectiveData(emptyDir)
    assert(output.isEmpty)
  }

  it should "return an empty string if asked to create FST strings from a non-existent directory" in   {
    val emptyDir = File("jvm/src/test/resources/no-fst")
    val fst = IrregAdjectiveDataInstaller.fstForIrregAdjectiveData(emptyDir)
    assert(fst.isEmpty)
  }
  ...
  it should "composite data from multiple sources" in  {
    ...
    mkdirs(targetDir)
    assert(targetDir.exists, "IrregAdjectiveDataInstallerSpec: could not create " + targetDir)
    val targetFile = targetDir / "lexicon-irregverbs.fst"
    //dataSource: File, corpusList: Vector[String], targetFile: File
    IrregAdjectiveDataInstaller(datasets, corpora, targetFile)
    assert(targetFile.exists, "IrregAdjectiveDataInstallerSpec: IrregAdjectiveDataInstaller did not create " + targetFile)

    val expectedLines = Vector("<u>proof\\.irradj1</u><u>lexent\\.n48627</u>totius<masc><gen><sg><pos><irregadj>","<u>proof\\.irradj2</u><u>lexent\\.n48627</u>totius<masc><neut><sg><pos><irregadj>")
